...
we are rewriting the UI in ../client/ to the current directory.

directory structure:
- art_direction/ <---- examples of the art direction
...
- Treat files in `art_direction/` as the source of truth for composition,
...
- Use the bundled fonts:
  - `Archive-Regular.otf` for bold/display and uppercase labels.
  - `stellar.otf` for editorial serif headings.
- Use the hand illustrations already in `assets/`; do not substitute stock
  imagery.
- Keep the flat editorial appearance. Avoid the rounded glass-card style from
  `../client/`.

### Home page decisions

- The hero uses a rusty-red grid background, two oversized hand illustrations,
  and the mixed Archive/Stellar `Natya Lab` wordmark.
- Hero body copy is monospace and keeps normal sentence casing.
- In the description, `hastas (hand gestures)` is yellow and
  `rasas (emotions)` is pink.
- Hero buttons use white `Explore`, yellow `Hastas`, and pink `Rasas`.
- `home.js` draws a native canvas network; do not add Vanta or another
  animation dependency.
- The network currently has 18 moving nodes. Five nodes carry the labels
  `storytelling`, `real meanings`, `self expression`, `beginner-friendly`, and
  `viniyogams`.
- Label backgrounds are almost transparent. Label anchor dots are hidden;
  other node dots remain visible. Labels are centered on their anchors.
- Nodes and labels must avoid the hero title, description, and buttons using
  the existing exclusion zone.
- Respect `prefers-reduced-motion`.

### About section decisions

- Keep the earlier spacious four-stage layout, not the compact
  `art_direction/part2.png` reproduction.
- The four stages are a flowchart with no numbers and no box outlines.
- Stages are connected by `2px` dotted pink arrows: horizontal on desktop,
  vertical on mobile.
- Keep the current text:
  - opening: traditional teaching through observation, repetition, and
    correction
...
- Maintain responsive layouts and accessibility basics, including meaningful
  alt text, `aria-current`, hidden decorative imagery, and reduced motion.
